...
listItem* createList ( int, int* );/*создание списка. Возвращает его адрес, первый парметр кол-во ходов короля, а второй-флаг, 
отвечающий за состояние,"не бьл", короля в точке*/
void printList ( listItem* );// формальный параметр- адрес списка
...
отвечающий за состояние "был", ф-ия связана с указателем по горизонтали */
listItem* newItem ( int, int ); // добавление эл-та, параметры- координаты
